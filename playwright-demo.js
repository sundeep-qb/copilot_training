...
async function openPlaywrightDocs(options = {}) {
  ...
  try {
    ...
    // Assertion: Verify page has proper meta description for SEO
    const metaDescription = await page.locator('meta[name="description"]').getAttribute('content');
    if (!metaDescription || metaDescription.trim().length === 0) {
      throw new Error('Assertion failed: Expected page to have a meta description tag for proper SEO');
    }
    console.log(`✓ Assertion passed: Page has valid meta description (${metaDescription.substring(0, 50)}...)`);
